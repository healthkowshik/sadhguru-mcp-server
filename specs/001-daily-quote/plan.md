...
**Scale/Scope**: Single MCP server, 1 resource template, 1 convenience resource
...
| II. MCP-First Design | PASS | Feature is exposed exclusively as MCP resources (`sadhguru://daily-quote/{date}` and `sadhguru://daily-quote/today`). No side-channel APIs. |
...
| II. MCP-First Design | PASS | Two MCP resources registered via `@mcp.resource()`. Response schema documented in `contracts/mcp-resources.md`. No side-channel APIs. |
...
└── sadhguru_mcp_server/
    ...
    ├── server.py          # FastMCP instance + resource registration
